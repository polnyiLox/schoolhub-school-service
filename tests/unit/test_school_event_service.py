...
async def test_valid_event_is_created_by_actor(transaction_session, user):
    ...
    assert (
        await service.create(
            class_id,
            SchoolEventCreate(title="Exam", event_type=SchoolEventType.EXAM, starts_at=now),
            user,
        )
        is entity
    )
...
async def test_event_update_validates_effective_dates(transaction_session, user):
    ...
    with pytest.raises(InvalidSchoolEventDatesError):
        await SchoolEventService(transaction_session, repository, access).update(
            class_id, entity.id, SchoolEventUpdate(ends_at=now - timedelta(minutes=1)), user
        )
