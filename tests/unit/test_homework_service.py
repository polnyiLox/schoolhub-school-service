...
def homework(class_id, subject_id, text="old"):
    return HomeworkORM(
        ...
        assigned_date=date(2026, 9, 14), due_date=date(2026, 9, 15), text=text,
        created_by_telegram_id=1, updated_by_telegram_id=None,
    )
...
async def test_editor_can_create_homework(transaction_session, user):
    ...
    result = await tested.create(class_id, HomeworkCreate(subject_id=subject_id, assigned_date=date(2026, 9, 14), due_date=date(2026, 9, 15), text="old"), user)
...
async def test_admin_can_create_homework(transaction_session, admin):
    ...
    await tested.create(class_id, HomeworkCreate(subject_id=subject_id, assigned_date=date(2026, 9, 14), due_date=date(2026, 9, 15), text="task"), admin)
...
async def test_student_cannot_create_homework(transaction_session, user):
    ...
    with pytest.raises(ClassAccessDeniedError):
        await tested.create(uuid4(), HomeworkCreate(subject_id=uuid4(), assigned_date=date(2026, 9, 14), due_date=date(2026, 9, 15), text="task"), user)
...
async def test_due_date_before_assigned_is_rejected(transaction_session, user):
    ...
    with pytest.raises(InvalidHomeworkDatesError):
        await tested.create(class_id, HomeworkCreate(subject_id=subject_id, assigned_date=date(2026, 9, 15), due_date=date(2026, 9, 14), text="task"), user)
    repository.create.assert_not_awaited()
...
async def test_subject_from_another_class_is_rejected(transaction_session, user):
    ...
    with pytest.raises(SubjectDoesNotBelongToClassError):
        await tested.create(class_id, HomeworkCreate(subject_id=subject_id, assigned_date=date(2026, 9, 14), due_date=date(2026, 9, 15), text="task"), user)
...
async def test_text_update_creates_revision_in_transaction(transaction_session, user):
    ...
    repository.create_revision.assert_awaited_once_with(
        homework_id=entity.id, old_text="old", new_text="new", changed_by_telegram_id=user.telegram_id
    )
...
async def test_homework_event_contains_ids(transaction_session, user):
    ...
    entity = homework(class_id, subject_id)
    repository.create.return_value = entity
    await tested.create(class_id, HomeworkCreate(subject_id=subject_id, assigned_date=entity.assigned_date, due_date=entity.due_date, text=entity.text), user)
