...
async def test_duplicate_schedule_slot_is_rejected(transaction_session, admin):
    ...
    repository.get_slot.return_value = ScheduleEntryORM(
        class_id=class_id,
        subject_id=subject_id,
        weekday=0,
        lesson_number=1,
...
async def test_schedule_entry_update_rejects_invalid_time(transaction_session, admin):
    ...
    service, repository = dependencies(transaction_session, class_id, subject_id)
    repository.get_entry.return_value = ScheduleEntryORM(
        id=entry_id,
        class_id=class_id,
        subject_id=subject_id,
        weekday=0,
    ...
    with pytest.raises(InvalidScheduleTimeError):
        await service.update_entry(
            class_id, entry_id, ScheduleEntryUpdate(start_time=time(10)), admin
        )
...
async def test_replaced_and_added_require_subject(transaction_session, admin, kind):
    ...
    with pytest.raises(InvalidScheduleOverrideError):
        await service.create_override(
            uuid4(),
            ScheduleOverrideCreate(date=date(2026, 9, 14), lesson_number=2, override_type=kind),
            admin,
        )
...
async def test_cancelled_override_does_not_require_subject(transaction_session, admin):
    ...
    assert (
        await service.create_override(
            class_id,
            ScheduleOverrideCreate(
                date=entity.date, lesson_number=2, override_type=ScheduleOverrideType.CANCELLED
            ),
            admin,
        )
        is entity
    ...
    with pytest.raises(InvalidScheduleOverrideError):
        await service.create_override(
            ...
            ScheduleOverrideCreate(
                date=date(2026, 9, 14),
                lesson_number=2,
                override_type=ScheduleOverrideType.REPLACED,
                subject_id=subject_id,
                start_time=time(10),
            ),
            admin,
...
async def test_created_schedule_event_type_is_correct(transaction_session, admin):
    ...
    await service.create_entry(
        class_id,
        ScheduleEntryCreate(
            subject_id=subject_id, weekday=0, lesson_number=1, start_time=time(8), end_time=time(9)
        ),
        admin,
    )
