...
class ClassDayService:
    def __init__(
        ...
        event_repository: SchoolEventRepository,
    ...
    async def get(self, class_id: UUID, target_date: date, actor: CurrentUser) -> ClassDayRead:
        ...
        logger.debug(
            "Loading active homework from database: class_id=%s, date=%s", class_id, target_date
        )
        ...
        logger.debug(
            "Loading school events from database: class_id=%s, date=%s", class_id, target_date
        )
