...
from app.repositories import ClassMemberRepository, ScheduleRepository, SchoolClassRepository, SchoolEventRepository, SubjectRepository
from tests.integration.helpers import EVENT_START, LESSON_END, LESSON_START, create_class, create_subject
...
async def test_duplicate_member_constraint(session):
    ...
    with pytest.raises(IntegrityError):
        async with session.begin_nested():
            await repository.create(class_id=school_class.id, telegram_id=100, role=ClassMemberRole.EDITOR)
...
async def test_duplicate_schedule_slot_constraint(session):
    ...
    values = dict(class_id=school_class.id, subject_id=subject.id, weekday=0, lesson_number=1, start_time=LESSON_START, end_time=LESSON_END, room=None)
    ...
    with pytest.raises(IntegrityError):
        async with session.begin_nested():
            await ScheduleRepository(session).create_entry(
                class_id=school_class.id, subject_id=subject.id, weekday=0, lesson_number=1,
                start_time=LESSON_END, end_time=LESSON_START, room=None,
            )
    ...
    with pytest.raises(IntegrityError):
        async with session.begin_nested():
            await SchoolEventRepository(session).create(
                class_id=school_class.id, title="Exam", description=None,
                event_type=SchoolEventType.EXAM, starts_at=EVENT_START,
                ends_at=EVENT_START - timedelta(hours=1), created_by_telegram_id=1,
            )
